...
- `package.json` — dépendances `@anthropic-ai/sdk` (`api/reading.js`, `api/portrait.js`), `luxon` + `astronomy-engine` (`api/astral.js`, `api/transits.js`)

## Lecture générée par IA — comment ça marche
La fonction `generateAIReading(question, cards, positions)` dans `app.js` envoie `{question, cards, positions, profile?, history?}` en `POST` vers `/api/reading` — `positions` (les intitulés des positions du tirage choisi, ex. « Défi », « Résultat ») donne à l'IA le sens de chaque carte dans les tirages autres que le tirage général ; `profile` et `history` sont deux résumés optionnels de personnalisation (voir « Profil astral » et « Journal intelligent » plus bas). Quand un prénom est enregistré, l'IA est aussi invitée à s'adresser à la personne par son prénom une fois dans la lecture, jamais de façon systématique. Cette fonction serverless (`api/reading.js`), agnostique du type de tirage (1 à 12 cartes, voir `SPREADS` dans `app.js`) :
...
- Bibliothèque de 89 symboles et nombres, triés par ordre alphabétique, tous cliquables et reliés aux cartes/divinités ; position de scroll préservée à la navigation. 53 symboles (plantes, animaux, objets, attributs des dieux) ont en plus un texte mythologique détaillé (`lore`, plusieurs paragraphes) affiché dans leur fiche sous « Aux origines du symbole »
