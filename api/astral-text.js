...
// localStorage), jamais régénéré tant que le profil n'est pas modifié — voir
// ensureAstralText() dans app.js, même logique que /api/portrait.
//
...
module.exports = async function handler(req, res) {
  ...
  if (deity) {
    const contributors = Array.isArray(deity.contributors)
      ? deity.contributors.filter((c) => c && typeof c.label === "string" && typeof c.sign === "string").map((c) => `${c.label} en ${c.sign}`).join(", ")
      : "";
...
${deity ? `- tutelaryReason : 2 à 3 phrases qui expliquent pourquoi CETTE figure mythologique précise résonne avec CE thème, en t'appuyant sur les placements qui ont le plus contribué au choix et sur ce que cette figure représente mythologiquement. N'explique jamais le calcul (poids, score) lui-même, seulement le sens. Ne remets jamais en cause le choix de la figure : explique-le, ne le questionne pas.` : ""}
