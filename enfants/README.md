...
## Contenu
21 fiches (les 12 Olympiens + Hadès, Perséphone, Héraclès, Pan, Hestia, Circé, Hygie, Hécate et Iris), chacune avec un nom, un titre, un domaine, un symbole, une description et une « info rigolote », pensées pour un enfant de 6 ans (pas de scène violente ou effrayante). Les 21 ont désormais un portrait et un attribut illustrés (`PORTRAIT_IMAGES`/`ATTRIBUTE_IMAGES`).

Pour les 4 derniers ajoutés (Circé, Hygie, Hécate, Iris), les portraits sont volontairement recadrés pour ne PAS montrer l'attribut que le personnage tient dans l'illustration d'origine (chaudron, coupe-serpent, flambeau, écharpe arc-en-ciel...) : sinon le jeu « Retrouve les attributs » serait trop facile, l'attribut étant déjà visible sur le portrait à associer.
...
- **Objets cachés** — vraie chasse aux symboles façon « cherche et trouve » : une grande scène illustrée (`assets/objets/`) et une liste de symboles à repérer, présentée sous forme de vignettes cliquables (pas seulement du texte, pour les enfants qui ne savent pas encore lire). Chaque vignette est un recadrage exact de la scène elle-même autour du symbole (`drawHuntThumb` dans `game-objets.js`, dessiné sur un `<canvas>` une fois l'image chargée) — jamais une icône générique, donc toujours reconnaissable. Il faut d'abord choisir un symbole dans la liste, puis cliquer sur son emplacement dans la scène : cliquer au hasard sans avoir choisi, ou cliquer au bon endroit pour un autre symbole que celui choisi, ne valide rien (juste un halo rouge bref). Les coordonnées de chaque symbole (`HIDDEN_SCENES` dans `game-objets.js`) sont en pourcentage de l'image, repérées à l'œil sur la scène. Pour ajouter une scène : déposer l'image, puis noter les coordonnées de chaque symbole (une grille de repérage aide beaucoup, voir l'historique du projet). Limité à des symboles réellement issus de la mythologie grecque (pas d'œil protecteur/nazar, qui n'en fait pas partie) — ce symbole reste malheureusement visible dans le décor peint des scènes existantes (`jardin.jpg`, `etagere.jpg`), n'ayant pas d'outil de retouche d'image pour l'effacer ; il n'est en tout cas plus proposé comme symbole à trouver.
...
- **Mémo** — retrouver les deux cartes identiques (portrait illustré, `PORTRAIT_IMAGES` dans `data.js`), en 6 ou 8 paires, sans texte sur les cartes.
- **Retrouve les attributs** — associer chaque carte-attribut (`ATTRIBUTE_IMAGES`) au portrait du bon dieu (clic sur l'attribut puis sur le portrait), par manches de 6, sur les 21 dieux.
...
- `data.js` — fiches des 21 dieux/héros (`GODS`), `PORTRAIT_IMAGES`/`ATTRIBUTE_IMAGES` (les 21), `shuffleArray`/`randomGods`
